...
const createMessageListener = (() => {
  ...
  return (userId, handleIncomingMessage) => {
    ...
    iStream.on('pending', () => {
      console.log('pending connection to groupme...');
    });
    ...
    iStream.on('status', (message, payload) => {
      console.log('status event from groupme', message, payload)
    });
    iStream.connect();
  };
})();
...
const extractInfo = (message) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (data) {
      ...
      if (messageData && messageData.sender_type !== 'bot' && groupId === messageData.group_id) {
